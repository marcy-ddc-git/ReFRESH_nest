...
This script will perform the following:
	...
	Remove the database from the AG if no errors during processing commands
...
IF @refreshAGname = @valid_AG
  ...
  */
  ...
  /* Remove the database from the AG if it is a valid database and run mode = 1
  */
	  IF @database_name = @valid_database_name AND @runAGrefresh = 1 
		...
		BEGIN TRY
		  ...
		  SET @sql_command = 'ALTER AVAILABILITY GROUP [' + @refreshAGname + '] REMOVE DATABASE [' + @database_name + '] ;' ;
		  EXEC (@sql_command) ;
